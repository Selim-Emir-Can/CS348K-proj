...
def run_single_game(cfg, players_spec, seed: int, max_turns: int = None,
                    on_turn=None, player_kwargs_list=None) -> dict:
    ...
    Args:
      ...
      player_kwargs_list: optional list of dicts, same length as
                    players_spec, of extra kwargs forwarded to each player
                    constructor (e.g. ``decision_log_path`` for LLMPlayer).
                    Default ``None`` preserves the original construction
                    behaviour exactly.
    ...
    players = []
    for i, (name, settings, cls_name) in enumerate(players_spec):
        cls = _PLAYER_CLASSES.get(cls_name, Player) if cls_name else Player
        extra = (player_kwargs_list[i] if player_kwargs_list else None) or {}
        p = cls(name, settings, **extra) if extra else cls(name, settings)
