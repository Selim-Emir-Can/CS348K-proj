...
score by approximately $45\%$ over the default board at both 2 and
3 players. Single-objective ablations confirm that every term in the
...
\begin{subfigure}[t]{0.48\linewidth}\centering
  ...
  \caption{3 players}
\end{subfigure}
\caption{\textbf{Problem framing drives most of the lift; the GA
edges out random search at 3p but matches it at 2p.} Best-so-far
composite score (lower is better) over the candidate-evaluation
budget, six runs per player count: random search (400 iters), the
combined-objective GA (population $20\times 20$ generations, 362
evaluations after 2-elitism), and the four single-objective
ablations (\textsl{abl\_fair, abl\_len, abl\_draw, abl\_money}). At
2p the GA and random search converge to the same level
($0.7315$ vs.\ $0.7216$, an essentially tied result); at 3p the GA
finishes $\sim 10\%$ ahead of random ($0.7157$ vs.\ $0.7979$). The
much larger lift over the default-board reference (the $\sim 0.45$
absolute gap, $\sim 45\%$ relative) is what carries the headline:
the choice of objective and evaluator drives most of the
improvement, with optimiser cleverness adding a smaller,
regime-dependent margin. The ablation runs reach much lower scores
because each minimises only one term of the composite, previewing
the non-redundancy claim made by Figure~\ref{fig:multipliers}.}
\label{fig:conv}
\end{figure}
...
\addlinespace
GA-2p winner     & 2p & \textbf{0.784} & \textbf{0.193} & 0.980 &  54.6 &  0.9 &  82.4 \\
GA-2p winner     & 3p & 0.716 & 0.287 & 0.510 &  60.6 &  1.9 & 154.2 \\
\addlinespace
GA-3p winner     & 2p & 0.873 & 0.269 & 0.980 &  55.3 &  1.1 &  76.2 \\
GA-3p winner     & 3p & \textbf{0.677} & 0.241 & 0.560 &  62.0 &  4.9 & 141.5 \\
\bottomrule
\end{tabular}
\caption{\textbf{Cross-player-count evaluation at $n{=}1000$ games
per cell, $66$-dim keep-mask design space} (source:
\texttt{logs/optimizer\_v2/cross\_eval\_mask.json}). Wilson $95\%$
...
($0.784$ vs $0.873$), GA-3p winner at 3p ($0.677$ vs $0.716$). Both
designs improve over the default by approximately $45\%$ in their
training regime ($1.463 \!\to\! 0.784$ at 2p, $1.230 \!\to\! 0.677$
at 3p) and generalise to within $\sim 10\%$ in the off-regime
...
$46\%$ on its training regime ($1.463 \!\to\! 0.784$ at 2p) and is
the best 2p design in the table; the GA-3p winner improves by $45\%$
on its training regime ($1.230 \!\to\! 0.677$ at 3p) and is the best
...
$0.716$ (vs.\ GA-3p's $0.677$, a $5.4\%$ relative gap), and the
GA-3p winner under 2p eval scores $0.873$ (vs.\ GA-2p's $0.784$, a
$11\%$ gap). The cleanest reading is that the GA finds a *training-
regime-specialised* optimum, with off-regime performance robust
enough that a single design from either training regime could ship
to either player count without disaster, but matching the design to
the deployment regime is worth $\sim 5\!-\!10\%$ on the composite.

The second finding is that \emph{optimisation-set overfit is small
on both designs}. At the $n{=}100$ optimisation budget the GA-2p
winner scored $0.732$ on its training matchup set; at $n{=}1000$
deployment it scores $0.784$, an overfit of $+0.052$
(Table~\ref{tab:overfit}). The GA-3p winner scored $0.716$ at
training and $0.677$ at deployment, a small \emph{negative} gap of
$-0.039$ (deployment is fractionally better than training, well
within the noise produced by different seed populations between the
two evaluation budgets). Both gaps are small relative to the
headline improvement over the default ($\sim 0.65$ on the 2p score
scale, $\sim 0.55$ on 3p, both about $45\%$ relative), so the
...
\midrule
GA-2p winner & $0.732$ & $0.784$ & $+0.052$ \\
GA-3p winner & $0.716$ & $0.677$ & $-0.039$ \\
\bottomrule
\end{tabular}
\caption{\textbf{Optimisation-set vs.\ deployment-set composite
score, on each design's own training regime.} Header arrows:
$\downarrow$ means lower is better. The GA-2p winner has a small
$+0.052$ overfit to its $100$-game training matchup set; the GA-3p
winner has a $-0.039$ \emph{negative} overfit (deployment is
fractionally better than training, within the noise produced by
the different seed populations between $n{=}100$ and $n{=}1000$).
Both gaps are small relative to the headline win over the default
($\sim 0.65$ on the score scale). Training number is the best
score in the GA's own \texttt{evals.jsonl}, deployment number is
the corresponding row of Table~\ref{tab:cross}.}
\label{tab:overfit}
\end{table}
...
on player count.} Each marker is a candidate evaluated by the GA
combined-objective run on the $66$-dim keep-mask design space. The
x-axis is mean pair-fairness $|\Delta\,\text{win rate}|$ (lower is
fairer); the y-axis is mean rounds per game; the colour encodes
draw rate. The lower-left region is the simultaneously fair, fast,
and decisive corner of the design space; the combined-objective
optimum lies near its knee. The 2p sweep reaches mean rounds as low
as $\sim 30$ (the keep-mask shrinks the lap), while the 3p sweep
bottoms out closer to $\sim 50$ rounds. The two clouds have visibly
different shapes, which is what makes the cross-evaluation finding
...
metrics aggressively, and per-pair strategy fairness modestly}. Mean
$|W-0.5|$ across the full $30\times 30$ matrix drops from $0.21$
(default) to $0.17$ (GA-2p winner) — a $21\%$ relative reduction in
per-pair asymmetry across the entire pool, even though the optimiser
only ever saw 10 sampled matchups during search. The system-level
metrics improve by larger relative margins: game length falls from
$103.9$ to $54.6$ rounds (a $-47\%$ drop), draw rate from $7.4\%$ to
$0.9\%$ ($-88\%$), and inter-player money transfer rises from $49.7$
to $82.4$~\$/round (closer to the target of $100$). The optimiser
shifted pacing, decisiveness, and interactivity by larger relative
amounts than it shifted strategy-level skill ordering — board design
is a stronger lever for the former than the latter.
...
pairs on the GA-2p winner are \textsl{Trader} vs.\
\textsl{RailroadKing} ($100\%$/$0\%$) and \textsl{RailroadKing}
losing $0\%$/$100\%$ against \textsl{Sampled\_19},
\textsl{Sampled\_18}, \textsl{Sampled\_16}, and \textsl{Sampled\_13}.
No setting of cost multipliers, rent multipliers, or keep-mask bits
eliminates these; they are intrinsic to a strategy that refuses to
buy $22$ of $28$ ownable properties, and the GA cannot reach them
through the design space. This is the substantive negative result of
the section: board design is a lever for system-level properties
(length, draws, interactivity) and for population-averaged fairness,
but agent-level dominance on these specific matchups is structural
to the strategy pool. Closing those residual gaps requires a
strategy-side intervention (rule changes, matchmaking, or pool
curation), not more search over the design space.
...
(default) to $0.17$ (GA-2p winner) — a $21\%$ relative reduction in
per-pair asymmetry. The GA improved system-level metrics (length,
draws, transfer) by larger relative margins; board design is a
stronger lever for those than for strategy-level skill ordering.}
\label{fig:heatmap}
\end{figure}
