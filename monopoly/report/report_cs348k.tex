...
score by approximately $40\%$ over the default board at both 2 and
...
\begin{subfigure}[t]{0.48\linewidth}\centering
  ...
  \caption{3 players}
\end{subfigure}
\caption{\textbf{Optimisation works, and each composite term is
optimisable in isolation.} Best-so-far composite score (lower is
better) over the candidate-evaluation budget, six runs per player
count: random search, the combined-objective GA, and the four
single-objective ablations (\textsl{abl\_fair, abl\_len, abl\_draw,
abl\_money}). The combined-objective GA finishes $\approx 16\%$
ahead of random search at both player counts; the ablation runs
reach much lower scores because each minimises only one term of the
composite, which previews the non-redundancy claim made by
Figure~\ref{fig:multipliers}.}
\label{fig:conv}
\end{figure}
...
\addlinespace
GA-2p winner     & 2p & \textbf{0.817} & \textbf{0.237} & 0.960 &  62.5 &  1.3 &  74.7 \\
GA-2p winner     & 3p & \textbf{0.724} & 0.251 & 0.570 &  66.8 &  1.2 & 142.4 \\
\addlinespace
GA-3p winner     & 2p & 0.924 & 0.294 & 0.990 &  63.8 &  0.4 &  66.0 \\
GA-3p winner     & 3p & 0.787 & 0.361 & 0.610 &  64.4 &  1.1 & 127.0 \\
\bottomrule
\end{tabular}
\caption{\textbf{Cross-player-count evaluation at $n{=}1000$ games
per cell, $66$-dim keep-mask design space} (source:
\texttt{logs/optimizer/cross\_eval\_mask.json}). Wilson $95\%$ CIs
on individual win rates are $\pm 3$pp. Header arrows: $\downarrow$
means lower is better; $(\!\to\!T)$ means the target value is $T$
and deviation in either direction is penalised. Bold cells mark
the best (lowest) value within each evaluation regime. Both
optimised designs improve over the default by roughly $40\%$ in
either regime; the GA-2p winner has a small but consistent advantage
over the GA-3p winner on both score and fairness in both regimes.}
\label{tab:cross}
\end{table}

The first finding is that \emph{both designs generalise across
player counts}. Neither catastrophically fails on the off-regime:
the GA-2p winner improves the composite by $44\%$ on its training
regime ($1.463 \!\to\! 0.817$ at 2p) and $41\%$ on the off-regime
($1.230 \!\to\! 0.724$ at 3p); the GA-3p winner improves by $37\%$
on the off-regime ($1.463 \!\to\! 0.924$ at 2p) and $36\%$ on its
training regime ($1.230 \!\to\! 0.787$ at 3p). Within the run, the
GA-2p winner has a small but consistent advantage over the GA-3p
winner in both regimes (score $0.817$ vs.\ $0.924$ at 2p, $0.724$
vs.\ $0.787$ at 3p; fairness $0.237$ vs.\ $0.294$ at 2p, $0.251$
vs.\ $0.361$ at 3p). With a single GA seed per regime we cannot
claim that 2p training \emph{systematically} produces stronger
universal designs; what the data does support is the weaker, more
useful claim for a designer: a board optimised under one player
count is not specialised to the point of failure under the other.

The second finding is that \emph{optimisation-set overfit is small
on both designs, and roughly twice as large at 2p as at 3p}. At the
$n{=}100$ optimisation budget the GA-2p winner scored $0.728$ on
its training matchup set; at $n{=}1000$ deployment it scores
$0.817$, an overfit of $0.089$ (Table~\ref{tab:overfit}). The
GA-3p winner scored $0.753$ at training and $0.787$ at deployment,
an overfit of $0.034$. Both gaps are small ($\leq 12\%$ of the
deployment score), and the headline improvement over the default
($\sim 0.65$ on the score scale, or $\sim 40\%$ relative) is several
times larger than either overfit, so the directional improvement
survives re-evaluation. The factor-of-two contrast between the two
designs is consistent with 3p evaluations carrying more
strategy-vs-strategy interactions per game and therefore being
intrinsically less noisy at the same budget.
...
\midrule
GA-2p winner & $0.728$ & $0.817$ & $+0.089$ \\
GA-3p winner & $0.753$ & $0.787$ & $+0.034$ \\
\bottomrule
\end{tabular}
\caption{\textbf{Optimisation-set vs.\ deployment-set composite
score, on each design's own training regime.} Header arrows:
$\downarrow$ means lower is better. The GA-2p winner has a $0.089$
overfit to its $100$-game training matchup set; the GA-3p winner
has $0.034$. Both gaps are small relative to the headline win over
the default ($\sim 0.65$). The training number is the best score in
the GA's own \texttt{evals.jsonl}, the deployment number is the
...
on player count.} Each marker is one of the candidates evaluated by
the GA (combined-objective and the four ablations, pooled). The
x-axis is mean pair-fairness $|\Delta\,\text{win rate}|$ (lower is
fairer); the y-axis is mean rounds per game; the colour encodes
draw rate. The lower-left corner is the simultaneously fair, fast,
decisive corner of the design space; the combined-objective
optimum lies on the knee of that corner. The 2p sweep ranges over
fairness up to $0.7$ and the 3p sweep only up to $0.6$, and the
shape of the cloud differs between the two: a board on one frontier
does not in general lie on the other, which is what makes the
cross-evaluation finding (Table~\ref{tab:cross}) a property of the
