...
used for all four runs (Phase~C and the LLM-driven GA, both at 2p
and 3p).
...
\paragraph{Search hyperparameters (canonical v3 protocol,
...
\texttt{scripts/rerun\_llm\_phase\_c\_v3.bat} (LLM Phase~C,
...
hallucination rate below $1\%$. We pre-declared a stronger
validation path (human playtest in Phase B) that is out of scope
for this checkpoint. The full statement is in
Section~\ref{sec:bg}.
...
Seven experiments, each with an explicit success criterion (full
detail in \texttt{monopoly/CHECKPOINT\_1.md}):
\begin{enumerate}[leftmargin=1.4em,itemsep=1pt]
