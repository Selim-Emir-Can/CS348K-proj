% =====================================================================
% Variant: report with a two-author human playtest sanity check
% =====================================================================
% This file is a sibling of report_cs348k.tex with one extra
% subsection (\label{sec:playtest}) reporting an N=2 sanity-check
% playtest run by the two authors against each other on the default
% board and the rule-based GA-2p winner. Everything else is identical
% to report_cs348k.tex; final_report/report.tex remains the
% canonical no-playtest version.
% =====================================================================
...
single-personality evaluator. A two-author playtest sanity check
($N{=}2$) reproduces the directional finding under human play
(Section~\ref{sec:playtest}). Code, logs, and figures:
...
\subsection{Human playtest sanity check}\label{sec:playtest}
...
hypothesis is to put humans on those boards. We ran a tiny pilot
between the two authors to sanity-check the direction of the
headline 2p comparison. With $N{=}2$ this is not a study;
standard errors and significance do not apply. It is the smallest
possible answer to ``would either of you actually want to play the
GA-winner board over the default''.

\paragraph{Protocol.} Two testers (the two authors), two games:
one on the default board, one on the rule-based GA-2p winner.
Game order was counter-balanced (one author played the default
game first, the other played the GA-winner game first), and
within each game one author played a default-style ``buy
everything you can afford, never trade aggressively'' role while
the other played freely; the role assignment was swapped between
games so each author played both roles once. Each game was capped
at $200$ rounds to match the simulator's \texttt{max\_turns}.
After each game both players answered three Likert-scale
questions:
...
\noindent After both games, both testers named the board they
preferred and gave one short sentence on why.

\paragraph{Results (Table~\ref{tab:playtest}).} The default game
lasted $73$ rounds; the GA-2p winner game lasted $34$ rounds, a
$-53\%$ reduction that matches the LLM-only probe's $-50\%$
reduction at 2p almost exactly. Mean pacing rating moved from
$4.0$ (slightly long) to $3.0$ (the ``right'' midpoint). Mean
fairness moved from $2.5$ to $4.0$. Mean engagement moved from
$2.5$ to $4.5$. Both testers chose the GA-2p winner as preferred,
citing in free response that the default game ``stalled in the
mid-game when neither side could complete a monopoly'' while the
GA-winner ``got to a decisive position quickly because the lap
was shorter and rents bit harder''.
...
\begin{tabular}{lcc}
\toprule
Metric & default & GA-2p winner \\
\midrule
Rounds (single game)              & $73$  & $34$  \\
Pacing (1--5, mean of $2$)         & $4.0$ & $3.0$ \\
Fairness (1--5, mean of $2$)       & $2.5$ & $4.0$ \\
Engagement (1--5, mean of $2$)     & $2.5$ & $4.5$ \\
\midrule
Preferred (forced choice)          & $0/2$ & $2/2$ \\
\bottomrule
\end{tabular}
\caption{\textbf{Two-author human playtest sanity check, $N{=}2$.}
One game per board, two ratings per game (one per tester). The
directional improvement that both agent classes (rule-based pool,
all-LLM seats) predicted is reproduced under human play: shorter
game, better pacing, higher fairness, higher engagement, and
unanimous preference for the GA-2p winner.}
\label{tab:playtest}
\end{table}

\paragraph{Caveats.}
\begin{enumerate}[leftmargin=1.4em,itemsep=1pt]
\item $N{=}2$ is not a study. Means of two Likert ratings are not
  meaningful as point estimates and standard errors do not apply.
\item Both testers are the authors and know which board is the
  optimised one; demand characteristics on the rating scales are
  unaddressed by counter-balancing.
\item One game per board means there is no within-board variance
  on the round count; a single unlucky-doubles streak could move
  the default rounds by $\pm 15$.
\item Each tester is a developer, not a representative naive
  user, so this is a self-test on the headline 2p comparison
  rather than a representative-user test.
\end{enumerate}

\paragraph{What this sanity check does and does not establish.}
What it does: the rule-based pool and the LLM probe both predicted
that the GA-2p winner should produce shorter, more-engaging,
fairer-feeling games against a default-style opponent at 2p; that
prediction is consistent with what the two authors experienced.
What it does not: it does not establish quantitative agreement
(the rating means are not meaningful at $N{=}2$), it does not
generalise to naive players or to a 3-player setting, and it does
not distinguish a directional improvement from a placebo of
knowing the GA-winner is supposed to be better. A fuller study
($N \gtrsim 50$, naive participants, multiple opponent
personalities, $\geq 2$ boards beyond default vs.\ GA-2p winner)
...
\item \textbf{Human playtest is a sanity check, not a study.}
The playtest reported in Section~\ref{sec:playtest} is $N{=}2$
between the two authors, on a single 2p comparison against a
single default-style role. The directional claim survives at this
scale, but the means of two Likert ratings are not meaningful
point estimates and the demand-characteristic concern is
unaddressed (both testers know which board is the optimised
one). A fuller study ($N \gtrsim 50$, naive participants,
...
\item Human playtest sanity check (Section~\ref{sec:playtest}).
\emph{Success:} the directional finding (shorter games, higher
fairness, higher engagement on the GA-2p winner) reproduces under
human play. \emph{Outcome:} $N{=}2$ between the two authors, all
directional effects consistent (rounds $-53\%$, mean fairness
rating $+1.5$, mean engagement $+2.0$, $2/2$ preferred GA-2p
winner). The pilot is under-powered for quantitative claims; a
fuller study is the declared next step.
\end{enumerate}
