...
single-personality evaluator. We did not run a human playtest;
Section~\ref{sec:playtest-shortlist} publishes the curated 4-board
shortlist that would form the pilot, with the design hypothesis
each board is intended to probe. Code, logs, and figures:
...
\subsection{Playtest shortlist (deferred)}\label{sec:playtest-shortlist}

We did not run a human playtest within the project window. Rather
...
playtest in which subjects cannot distinguish board~2 from
board~1, or rate board~4 the same as board~1 on engagement,
would directly undercut the corresponding claim. We frame the
...
\item \textbf{Human playtest is deferred, not completed.}
No human playtest was run within the project window. The
shortlist in Section~\ref{sec:playtest-shortlist} curates 4
boards from the GA results --- default, GA-2p combined winner,
GA-3p combined winner, money-only 2p optimum --- and names the
design hypothesis each one is intended to probe, with a
proposed pilot protocol ($N{=}8$, counterbalanced, Likert plus
forced-choice). The agent-side results stand or fall on their
own; the shortlist is the falsification plan a future pilot
would execute.
...
plus forced-choice). No human playtest was run within the project
window; the shortlist is published as a falsification plan, not a
completed study.
\end{enumerate}
