...
its $n{=}5$ training overfit to. A small $N{=}10$ two-player pilot
(Section~\ref{sec:playtest-pilot}) confirms all four trained effects
under human control: the GA winner finishes in $50$ rounds on
average against $131$ for the default, circulates $\$66$/round
against $\$39$, never truncates ($0\%$ vs.\ $40\%$ draw rate), and
delivers a perfect $5$--$5$ win split between the two players
($|WR_{P_1}-WR_{P_2}|{=}0.00$) against $0.60$ for the default. The
fuller multi-subject playtest (Section~\ref{sec:playtest-shortlist})
that
...
resolves more reliably, and is perfectly balanced under human
control.} Per-board mean over ten games at matched seeds; dots are
individual games. \textbf{Top-left:} the GA winner finishes in $50$
rounds on average against $131$ for the default, four of whose ten
games hit the $200$-turn cap with no winner (red crosses).
\textbf{Top-right:} inter-player money transfer per round is higher
on the GA winner ($\$66$/r) than on the default ($\$39$/r), moving
toward the $\$100$/r target. \textbf{Bottom-left:} the default
truncated in $4$ of $10$ games; the GA winner truncated in $0$ of
$10$. \textbf{Bottom-right:} win-rate spread $|WR_{P_1}-WR_{P_2}|$
is $0.60$ on the default (Player~1 won all $6$ decisive games)
against $\mathbf{0.00}$ on the GA winner (a perfect $5$--$5$
Player~1/Player~2 win split). $N{=}10$ per board, two players.}
\label{fig:playtest}
\end{figure}
...
We read this pilot as directional support consistent with the
agent-predicted effects, not a fully powered measurement. The
fairness spread of $0.00$ at $N{=}10$ is the strongest possible
outcome at this sample size, but a larger study across many
subjects would still be needed to rule out single-pair idiosyncrasy
in the strategic style. The pacing, money-circulation, and
decisiveness effects are large enough that they are unlikely to be
artefacts of either subject or seed. What this pilot does show is
that all four trained effects (pacing, money circulation,
decisiveness, and win-balance) move in the predicted direction, at
the predicted magnitude, on contact with human play. The fuller
multi-subject playtest that would substantiate these effects across
the broader population remains deferred; we specify it next.
...
Only a small $N{=}10$ two-player pilot was run
...
plus forced-choice). A small $N{=}10$ two-player pilot
(Section~\ref{sec:playtest-pilot}) confirms all four trained effects
(pacing, money circulation, decisiveness, and win-balance) on this
single subject pair; the fuller multi-subject playtest is published
as a falsification plan, not a completed study.
\end{enumerate}
