...
single-personality evaluator. A small $N{=}2$ self-play pilot
(Section~\ref{sec:playtest-pilot}) confirms the predicted pacing and
money-circulation effects under human control; the fuller
multi-subject playtest (Section~\ref{sec:playtest-shortlist}) that
...
\subsection{Human playtest pilot ($N{=}2$, self-play)}\label{sec:playtest-pilot}

As a reality check on whether the agent-optimized boards behave as
predicted under human play, one author played both seats of four
two-player boards: the default, the GA-2p combined winner
(Section~\ref{sec:baseline}), and the length-only and money-only
ablation optima (Section~\ref{sec:ablation}). Each board was played
twice under matched RNG seeds ($0$ and $42$), a common-random-numbers
pairing across boards so that a difference reflects the board rather
than luck. The two per-game observables the search optimizes that are
legible from a single transcript, game length and inter-player money
transfer, were recovered from the game logs with
\texttt{scripts/human\_game\_stats.py}.

\begin{figure}[h!]
\centering
\includegraphics[width=\linewidth]{fig_human_playtest.png}
\caption{\textbf{The agent-optimized boards play faster and circulate
more money under human control.} Per-board mean over two self-play
games at matched seeds ($0$ and $42$); dots are the individual games.
Left: the three optimized boards finish in $31$--$36$ rounds on
average against $139$ for the default, which in one of its two games
hit the $200$-turn cap with no winner (red cross). Right: inter-player
money transfer per round is higher on the optimized boards
($60$--$73$ \$/r) than on the default ($48$ \$/r), moving toward the
$100$ \$/r target. $N{=}2$ per board, a single subject controlling
both seats.}
\label{fig:playtest}
\end{figure}

Figure~\ref{fig:playtest} summarizes the eight games. The three
optimized boards finish in $31$--$36$ rounds on average against $139$
for the default, and in one of its two games the default failed to
resolve at all, hitting the $200$-turn cap with no winner. All six
optimized games ended decisively by bankruptcy; the default resolved
only one of two. Inter-player money transfer per round was higher on
the optimized boards on average ($60$--$73$ \$/r) than on the default
($48$ \$/r).

We read this pilot as directional support, not measurement. With
$N{=}2$ games per board played by a single subject controlling both
seats, it cannot speak to fairness (a win-rate property across many
games and players) or to draw rate, and the ordering \emph{among} the
optimized boards is within single-game noise. What it does show, under
two matched seeds, is that the pacing and money-circulation effects
the GA was trained to produce survive contact with a human at the
controls. The fuller multi-subject playtest that would test the
subjective and fairness claims remains deferred; we specify it next.
...
Only a small $N{=}2$ self-play pilot was run
(Section~\ref{sec:playtest-pilot}): one subject played both
seats of four 2p boards under two matched seeds, which
directionally confirms the pacing and money-circulation effects
but cannot measure fairness or draw rate. The full multi-subject
playtest remains deferred. The shortlist in
...
plus forced-choice). A small $N{=}2$ self-play pilot
(Section~\ref{sec:playtest-pilot}) directionally confirms the pacing
and money-circulation effects; the fuller multi-subject playtest is
published as a falsification plan, not a completed study.
\end{enumerate}
