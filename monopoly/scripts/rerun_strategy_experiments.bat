...
REM
REM Total wall time: ~2-3 hours on a single CPU.
REM
REM Usage (from cmd.exe):
REM   monopoly\scripts\rerun_strategy_experiments.bat
REM
REM On finish, results land in:
REM   logs\optimizer_v2\summary.log            (start/end timestamps + per-run "starting"/"done" trail)
...
echo [%TIME%] starting %RUN% >> "%SUMMARY%"
python scripts/report_runs.py --runs %OUT%/random_2p_mask.jsonl %OUT%/ga_2p_mask.jsonl %OUT%/abl_fair_2p_mask.jsonl %OUT%/abl_len_2p_mask.jsonl %OUT%/abl_draw_2p_mask.jsonl %OUT%/abl_money_2p_mask.jsonl --out-dir %OUT%/reports_2p_mask > "%OUTBS%\%RUN%.log" 2>&1
if errorlevel 1 goto :error
echo [%TIME%] done    %RUN% >> "%SUMMARY%"

set "RUN=reports_3p_mask"
echo [%TIME%] starting %RUN% >> "%SUMMARY%"
python scripts/report_runs.py --runs %OUT%/random_3p_mask.jsonl %OUT%/ga_3p_mask.jsonl %OUT%/abl_fair_3p_mask.jsonl %OUT%/abl_len_3p_mask.jsonl %OUT%/abl_draw_3p_mask.jsonl %OUT%/abl_money_3p_mask.jsonl --out-dir %OUT%/reports_3p_mask > "%OUTBS%\%RUN%.log" 2>&1
if errorlevel 1 goto :error
echo [%TIME%] done    %RUN% >> "%SUMMARY%"
