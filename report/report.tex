...
on the GA-winner board), so the GA is implicitly optimising against an
``aggressive-buyer-only'' eval distribution. A board designed for an
aggressive evaluator should naturally have shorter games. (3) A
proper apples-to-apples test requires re-evaluating the LLM-driven
winner under the rule-based pool's 1{,}000-game protocol; we leave
that as a one-off cross-evaluation in future work.
