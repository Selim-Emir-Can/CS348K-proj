...
directional predictions across agent classes, and reserve scarce human
playtests for the designs where the automated signals are strongest or
most conflicted. Monopoly is our reproducible case study. A genetic
algorithm searches a 66-dim board design space (per-property cost/rent
multipliers + a keep-mask that structurally shortens the board) against
a composite of fairness, game length, draw rate, and inter-agent money
transfer; single-objective ablations and a
$2p\!\leftrightarrow\!3p$ cross-evaluation at $n{=}1000$ yield
$\sim$40\% score reductions over the default board. The novel claim is
not that the resulting Monopoly board is universally better, nor that
agents are accurate human substitutes. Rather, agent feedback can be
decision-useful when interpreted as a directional, comparative signal.
Phase A of our validation is complete: on a 16-cell mini board with
three pre-declared design knobs, two independent agent classes
(rule-based + a Qwen2.5-1.5B LLM) agree on direction for $8/12$ metric
comparisons and \emph{cross-confirm a counterintuitive finding}
(doubling salary increases draw rate from $50\%$ to $75$--$90\%$ rather
than reducing it). Phase B human validation is intentionally left as
the final test of transfer to real play.
Code:
...
We built a closed-loop game-design optimiser for Monopoly, but the main
result is the design workflow rather than the board it outputs. A
diverse parametric strategy pool evaluates candidate boards and improves
a combined fairness/length/draw/interactivity score by 32\%--46\%
($n{=}1000$) on both 2-player and 3-player versions, beating random
search by 16--17\% at equal evaluation budget. A second agent class
(Qwen2.5-1.5B) then lets us interpret those simulations as designer
signals: agreement cross-confirms robust effects, while disagreement
identifies where human playtesting has the highest value. The framework,
and its nine extracted MAS principles, generalises to environment design
problems where (i) the environment can be parameterised, (ii) a diverse
agent population can be assembled, and (iii) evaluation is cheap relative
to the search budget. Human validation remains the final test of whether
these directional signals transfer to real play.
