...
\author{Selim Emir Can \and Alaz Cig}
\date{2026-04-24}
...
\begin{abstract}
Game designers face combinatorial design spaces that human
playtesting cannot cover. Existing automated approaches score
candidate designs against a single trained agent or a single LLM,
but a single-agent evaluator cannot separate design effects from
agent-specific quirks, and naive LLM use produces prompt-conditioned
reasoning that drifts from the actual game state. We argue that
automated design feedback is decision-useful when treated as a
directional signal compared across multiple agent classes, with
deterministic per-field validation of the agent's claims about
state, and we instantiate that workflow on Monopoly as a
reproducible case study.

A genetic algorithm searches a 66-dim board design space (cost/rent
multipliers and a keep-mask) against a composite of fairness, game
length, draw rate, and inter-player money transfer, evaluated by a
30-strategy parametric pool. Single-objective ablations and a
$2p\!\leftrightarrow\!3p$ cross-evaluation at $n{=}1000$ improve the
composite score by approximately 40\% over the default board. A
second agent class (Qwen2.5-1.5B with a structured echo-validated
prompt) reproduces the same directional signal across 80 all-LLM
games on the optimised boards, with 0 model-side hallucinations
across 2{,}288 decisions. Re-running the GA with the LLM in the
evaluator role produces a board that beats the default but loses to
the rule-based winner under the diverse pool, with a fairness gap
(0.20 vs.\ 0.379) that diagnoses single-personality evaluator blind
spots. We conclude that the validator-and-retry stack around an
agent is part of the probe, multi-class agreement is the right unit
of evidence, and evaluator diversity is non-optional for
fairness-sensitive design.
Code: \url{https://github.com/Selim-Emir-Can/CS348K-proj}.
\end{abstract}
...
agent-assisted design optimisation. Our goal is not to ``solve'' Monopoly
but to demonstrate, on a reproducible setup, a designer-facing
beta-testing loop:
\begin{quote}
\textbf{Parameterise environment} $\to$ \textbf{evaluate against a diverse
agent population} $\to$ \textbf{compare agreement/disagreement across
agent classes} $\to$ \textbf{optimise or shortlist candidate designs}
$\to$ \textbf{send only the most informative cases to human playtest.}
\end{quote}

\paragraph{The contribution we are claiming.} The deliverable is not
the optimised board itself but the \emph{workflow for using imperfect
agents as design probes}. The guarantee we are after is weaker and
...
\centering
\textbf{Designer workflow}\\[3pt]
Design knobs
$\rightarrow$ agent populations
$\rightarrow$ metric table
$\rightarrow$ agreement/disagreement map
$\rightarrow$ candidate shortlist
$\rightarrow$ targeted human playtest\\[3pt]
\raggedright
\textbf{Agreement} increases confidence that a design effect is robust.
\textbf{Disagreement} identifies exactly where human playtesting has the
highest information value.
\end{minipage}}
\caption{\textbf{Agent-assisted beta testing as a decision workflow.}
The reusable artifact is not a Monopoly board but a procedure for
turning cheap agent simulations into a smaller, better-justified human
playtest queue.}
\label{fig:workflow}
\end{figure}

The deliverables of this report are an open-source closed-loop
optimisation codebase plus four conceptual contributions:
\begin{enumerate}[leftmargin=1.2em,itemsep=1pt]
\item \textbf{A multi-agent, multi-metric game-design loop}: candidate
boards are scored on fairness, length, decisiveness, and interactivity
rather than win rate alone.
\item \textbf{A designer-facing interpretation layer}: agent outputs are
treated as directional rankings and disagreement diagnostics, not as
literal human forecasts.
\item \textbf{A Monopoly case study}: GA search, ablations, heatmaps, and
cross-regime evaluation show that the loop produces internally
consistent design changes.
\item \textbf{A pre-declared validation path}: a completed mini-board
agent/LLM sanity check and a deferred human protocol make the transfer
claim falsifiable rather than post-hoc.
\end{enumerate}
...
\paragraph{Positioning vs prior work.} Search-based game design and
automated playtesting establish that parameters can be tuned against
computational objectives and that agents can expose dominant
strategies. Multi-agent RL typically fixes the environment and evolves
agents. Our setting sits between these lines: the \emph{environment}
is the search variable; the \emph{agent population} is fixed, diverse,
and intentionally imperfect. The novelty is the interpretation layer.
We do not ask whether any one agent is ``the human''; we ask whether
multiple agent classes agree on the direction of a design effect,
reserving human time for the cases where they disagree. The framing
applies to matchmaker design, auction tuning, platform-rules
optimisation, and other multi-agent systems whose designer instruments
the environment independently of the agents.
...
\paragraph{Board design space (66 dims).}
\begin{itemize}[leftmargin=1.2em,itemsep=0pt]
...
\item 22 binary \emph{keep-mask} bits, one per colour-group property.
      A zero bit deletes that property from the board, structurally
      shortening the lap rather than merely neutralising the space.
      Railroads and utilities are fixed.
\end{itemize}
...
maps a 66-dim vector to a fresh \texttt{GameConfig}: per-property
multipliers are applied to \texttt{cost\_base},
\texttt{cost\_house}, \texttt{rent\_base}, and \texttt{rent\_house};
properties whose keep-mask bit is zero are removed from the board
entirely, structurally shortening the lap (a legacy
FreeParking-substitution path is retained for the canonical-layout
rendering only). Identity vector $=$ the canonical default board.
...
\end{subfigure}
\caption{\textbf{Search convergence on the 66-dimensional board design
...
\section{Discussion: lessons for multi-agent system design}\label{sec:gen}
...
agents. We extract nine transferable principles, supported by specific
...
\item \textbf{Cross-agent agreement and disagreement are both useful.}
The Phase A salary-$\times 2$ knob is more credible because rule-based
agents and the LLM independently predict the same counterintuitive
direction: shorter games but more draws. The drop-Brown knob is useful
for the opposite reason: the two agent classes disagree sharply, which
turns it into a high-value human-test condition. \emph{Lesson}: a
designer should not collapse all agent feedback into one score too
early. Agreement identifies robust candidates; disagreement identifies
where human playtesting is most informative.
...
\textbf{No human-validation evidence yet.} Every Section~\ref{sec:eval}
result is internal to the agent loop. Section~\ref{sec:phase_a_results}
adds a second independent agent class (a Qwen2.5-1.5B LLM) and observes
that the two classes agree on direction for $8/12$ metric comparisons
across three pre-declared design knobs, including a non-obvious
counterintuitive finding (salary doubled $\to$ \emph{more} draws). This
is meaningful evidence that the agent loop is internally consistent,
but it is not yet evidence that the loop predicts human play. The
central project claim (agent feedback is directionally predictive of
real human play) therefore remains pending Phase B
(Section~\ref{sec:validation}); everything below is contingent on
those results.
...
\section{Validation plan and Phase A results}\label{sec:validation}

The central claim (agent feedback is directionally predictive of
real play) is operationalised by a three-phase validation plan with
pre-declared methodology, statistics, and falsification criteria.
Each phase generates a different kind of evidence about \emph{when}
agent feedback is decision-useful. As of this writing, Phase A and
Phase C are complete; Phase A is reported in
Section~\ref{sec:phase_a_results} and Phase C in
Section~\ref{sec:phase_c}. Phase B (human cohort) remains future
work with a concrete protocol committed below.
...
\subsection{Phase A: results}\label{sec:phase_a_results}

We ran each pre-declared knob on the 16-cell mini board with two agent
classes: \textsc{RuleBased} vs.\ \textsc{RuleBased} (\(n=30\) games per
condition) and \texttt{LLMPlayer}-with-Qwen2.5-1.5B-Instruct vs.\
\textsc{RuleBased} (\(n=12\) games per condition). All games used
\texttt{max\_turns}=200 and shared the same per-condition seed list
(variance reduction). Total wall-clock: \(\approx 17\) minutes.
Table~\ref{tab:phase_a} reports the default-vs-modified comparison
for each knob.
...
\toprule
Knob & Metric & RB default $\to$ mod & LLM default $\to$ mod & RB$\leftrightarrow$LLM \\
\midrule
\multirow{4}{*}{Salary $\times 2$} & rounds          & $71.9 \to 47.8$ ($\downarrow$) & $69.5 \to 39.7$ ($\downarrow$) & \checkmark \\
                                    & \textbf{draws}  & $\boldsymbol{50\% \to 90\%}$ ($\uparrow$) & $\boldsymbol{50\% \to 75\%}$ ($\uparrow$) & \checkmark \\
                                    & bankruptcies    & $0.50 \to 0.10$ ($\downarrow$) & $0.50 \to 0.25$ ($\downarrow$) & \checkmark \\
                                    & win-rate spread & $0.37 \to 0.03$ ($\downarrow$) & $0.33 \to 0.25$ ($\downarrow$) & \checkmark \\
\midrule
\multirow{4}{*}{Drop Brown}        & rounds          & $71.9 \to 67.7$ ($\approx$)    & $69.5 \to 46.1$ ($\downarrow$) & $\times$  \\
                                    & draws           & $50\% \to 53\%$ ($\approx$)    & $50\% \to 8\%$ ($\downarrow$)  & $\times$  \\
                                    & bankruptcies    & $0.50 \to 0.47$ ($\approx$)    & $0.50 \to 0.92$ ($\uparrow$)   & $\times$  \\
                                    & win-rate spread & $0.37 \to 0.33$ ($\approx$)    & $0.33 \to 0.58$ ($\uparrow$)   & $\times$  \\
\midrule
\multirow{4}{*}{Orange rent $\times 2$} & rounds       & $71.9 \to 51.6$ ($\downarrow$) & $69.5 \to 54.2$ ($\downarrow$) & \checkmark \\
                                         & draws        & $50\% \to 33\%$ ($\downarrow$) & $50\% \to 50\%$ ($\approx$)    & $\sim$    \\
                                         & bankruptcies & $0.50 \to 0.67$ ($\uparrow$)   & $0.50 \to 0.50$ ($\approx$)    & $\sim$    \\
                                         & win-rate spread & $0.37 \to 0.40$ ($\uparrow$) & $0.33 \to 0.00$ ($\downarrow$) & $\times$  \\
\bottomrule
\end{tabular}
\caption{\textbf{Phase A: pre-declared design-knob sweep on the
16-cell mini board.} Each cell shows default-board value $\to$
modified-board value, with arrows indicating direction of change.
The right-most column flags whether the rule-based and LLM agent
classes agree on the direction (\checkmark), partially agree
($\sim$), or disagree ($\times$) on that metric. Bold rows indicate
the central non-obvious finding cross-confirmed across both agent
classes.}
\label{tab:phase_a}
\end{table}
...
\item \textbf{Salary $\times 2 \to$ \emph{more} draws (cross-confirmed
non-obvious finding).} Both rule-based agents and the LLM independently
agree on direction across all four metrics. Game length shrinks $\sim$30\%
(\(72 \to 48\) RB; \(70 \to 40\) LLM); draw rate \emph{rises} sharply
(\(50\% \to 90\%\) RB; \(50\% \to 75\%\) LLM); bankruptcy rate collapses;
win-rate spread flattens. The mechanism is the same in both: doubling
salary makes cash accumulate faster than rents drain it, so bankruptcy
becomes rare and games end via the all-rich truncation rather than via a
decisive bankruptcy. Intuition predicts the opposite (``more salary
$\to$ faster game $\to$ fewer draws''); the agent loop falsifies that
prediction cheaply, and the falsification holds across two independent
agent populations. This is the kind of finding an automated
beta-testing tool is worth building for: surfacing structural patterns
intuition would not have predicted, on a substrate (the 16-cell mini
board) where each game costs milliseconds rather than hours.

\item \textbf{Drop Brown: agent classes genuinely disagree (and the
disagreement is itself a finding).} Rule-based reports the Brown
removal as essentially a no-op (rounds barely change, draw rate moves
\(50\% \to 53\%\), bankruptcy rate \(0.50 \to 0.47\)). The LLM reports it
as highly decisive (rounds \(70 \to 46\), draws collapse \(50\% \to 8\%\),
bankruptcies almost double \(0.50 \to 0.92\)). At \(n=6\) we wondered if
this was sample-size noise; at \(n=12\) the gap grew, so it is not
noise. The most plausible reading is that the LLM player is making
structurally different buy decisions when Brown is absent (likely
diverting cash that would have bought Brown into more aggressive bids
on remaining groups), whereas rule-based always-buy-everything is
indifferent to which low-cost groups exist. This is itself a workshop
finding: \emph{which agent population the designer chooses to validate
against changes the design conclusion}. A designer using only
rule-based agents would conclude ``Brown is irrelevant, drop it
freely''; a designer using only an LLM agent would conclude ``Brown is
critical to the game's pacing''. Phase B (humans) is the natural
tiebreaker, and this knob now becomes the highest-priority condition
for the human study.

\item \textbf{Orange rent $\times 2$: signal weaker than predicted.}
Rule-based sees the predicted bankruptcy increase; the LLM does not.
Suggests the Orange-rent-doubling design knob is less robust than the
ablation matrix in Section~\ref{sec:ablation} implied. Lower priority
for the human study.

\end{enumerate}

\paragraph{What Phase A establishes for the project claim.} The agent
loop \emph{is} decision-useful at the mini-board scale: 8 of 12
metric-direction comparisons across the three knobs match between two
independent agent classes, including the entire row for the
non-obvious salary-$\times 2$ finding. Where the two classes disagree
(drop Brown), the disagreement itself is diagnostic: it identifies
exactly the conditions where Phase B human data has the most
discriminative power. The validation plan's three-phase structure was
not redundant scoping: Phase A demonstrably surfaced a result a single
agent class could not have produced alone.
...
Qwen2.5-1.5B-Instruct model). The LLM is given a structured \texttt{STATE}
block containing the buy decision's full context (cash, property,
group, cost, base rent, own/opponent ownership counts, group size) and
asked to first emit an \texttt{ECHO} block copying every \texttt{STATE}
field verbatim, then a \texttt{REASON} sentence, then
\texttt{ANSWER: BUY} or \texttt{ANSWER: PASS}. The echo block is checked
deterministically against ground-truth \texttt{STATE}; on any mismatch
the model is given up to 4 corrective retries before its final answer
is used. Build, trade, and jail decisions fall through to the base
\texttt{Player} logic to keep latency manageable.

\paragraph{Setup.} 80 games total: 2 player counts × \{default board,
GA-winner board\} × 20 seeds, all seats are \texttt{LLMPlayer}. Greedy
decoding, \texttt{max\_new\_tokens=160}. Wall time 5h 14min on a single
11~GB GPU. Decision log is 2{,}288 LLM calls with full prompt, raw
response, parse path, timing, and echo-validation trace per call.

\paragraph{Cross-class directional agreement.} The headline result is
that the LLM, when used as both player and evaluator, reproduces the
rule-based GA's directional signal with almost no friction: GA-winner
boards are shorter and more cash-mobile in 2p (rounds $60.5\!\to\!46.8$,
transfer $50.1\!\to\!78.5$~\$/r) and 3p (rounds $76.0\!\to\!43.2$,
transfer $100.5\!\to\!127.4$~\$/r). Figure~\ref{fig:cross_class} plots
both metrics side by side. \emph{The LLM and the rule-based pool agree
on direction for all four cells of the comparison.} This is the
cross-class agreement Phase C was designed to surface.

\begin{figure}[h!]
\centering
\includegraphics[width=0.99\linewidth]{llm/fig_cross_class_agreement.png}
\caption{\textbf{Task 1 cross-class agreement.}
80 games of all-LLM play (Qwen2.5-1.5B, deterministic echo
validation) on the default board vs the rule-based GA winner. The
LLM reproduces the same directional signal the rule-based GA pool
produced: shorter games, more inter-player money flow on the
optimised board, in both 2p and 3p. Composite scores are not directly
comparable across evaluators (rule-based: 1{,}000 games × 10 strategy
matchups per candidate; LLM: 5 games per candidate), but the
\emph{per-component metrics} are unit-comparable.}
\label{fig:cross_class}
\end{figure}

\paragraph{Hallucination accounting.} Across 2{,}288 LLM calls the
deterministic echo validator flagged \emph{zero} mismatches. The model
copied \texttt{STATE} verbatim on every first attempt; no retries
fired. This is the post-fix number after a previous run of the same
pipeline (v1) revealed a validator bug; the original \texttt{int()}-only
parser failed on legitimate float drift in \texttt{Player.money}
(e.g.\ \$411.79999999999995 from rent-multiplier rounding). Once the
validator was extended to accept floats within \$0.50 of \texttt{STATE},
the pipeline produced 0/2{,}288 false positives.

\paragraph{Second-order finding: retry-induced fabrication.} The v1
run, by contrast, falsely flagged 100/2{,}304 calls (4.3\%). Tracing
those cases reveals an interesting failure mode: when the validator
incorrectly rejected the model's clean first-pass echo, the model
faithfully ran through the retry budget (4 attempts), and after
repeated ``you failed echo validation'' feedback it began \emph{making
up} numbers that didn't appear anywhere in \texttt{STATE} , 
e.g.\ ``I have \$1000, cost is only \$100'' on a turn where
\texttt{STATE} actually said cash=\$416.0, cost=\$180. We thus
observed 47 retry-induced fabrications in v1 that did not exist in v2.
The validator-and-retry stack is therefore not a neutral wrapper
around the LLM: when the validator is right, it nudges the model
toward correctness; when the validator is wrong, it nudges the model
toward fabrication. For the agent-as-design-probe writeup this is
evidence that the protocol around the LLM is part of the probe, not
just transport. Figure~\ref{fig:v1_v2_halluc} visualises the v1 vs v2
comparison with the per-board breakdown of spurious vs retry-induced
artefacts; v2's flat-zero pattern across the 2{,}288 calls confirms
the methodology when the validator is correctly implemented.

\begin{figure}[h!]
\centering
\includegraphics[width=0.99\linewidth]{llm/fig_v1_vs_v2_hallucination.png}
\caption{\textbf{Hallucination accounting (v1 vs v2).} The v1 run
flagged 100/2{,}304 calls; all of them caused by an int()-only
validator that couldn't parse legitimate float drift in
\texttt{Player.money}. 47 of those cases also produced retry-induced
fabrication after 4 corrective rounds. The v2 run with the fixed
validator: 0/2{,}288 across all four boards. The validator and the
retry policy are part of the probe; their failure modes show up in
the data the probe produces.}
\label{fig:v1_v2_halluc}
\end{figure}

\paragraph{Buy-rate slices.} Figure~\ref{fig:llm_slices} contrasts the
LLM's behaviour on the default board vs the GA-winner board. Three
patterns are worth highlighting. (1) The model is correctly more
cautious as cash falls (buy-rate slope vs cash bucket). (2) The
opponent-dominates slice drops sharply on the GA-winner board (0.30
default $\to$ 0.13 winner); the optimised board has rent multipliers
that punish buying into a contested group, and the model picks up the
signal from \texttt{base\_rent} alone. (3) The model becomes more
cash-aggressive at high balances on the GA-winner board (0.45 $\to$
1.00 at \$\geq$1500), consistent with the GA optimising for fast cash
circulation.

\begin{figure}[h!]
\centering
\includegraphics[width=0.99\linewidth]{llm/fig_llm_buy_rate_slices.png}
\caption{\textbf{LLM buy-rate slices, 2p only.} On the GA-winner board
the model becomes both more cash-aggressive at high balances
(\$\geq$1500: 0.45$\to$1.00) and more strict on contested groups
(opp\_dominates: 0.30$\to$0.13). Both shifts are coherent with the
board the GA selected: rents are amped, monopoly paths are scarcer,
and money is meant to circulate faster.}
\label{fig:llm_slices}
\end{figure}
...
three agree on the direction of an effect, we have strong evidence
that the design change is real. Phase C now provides two of those
three signals (rule-based + LLM); the human cohort remains future
work. With both automated agent classes agreeing on direction across
4/4 (board, player-count) cells, the rule-based GA's design choices
gain significant external validity from a fundamentally different
agent architecture.

\subsection{LLM-driven GA: optimising the board for the LLM
            evaluator}\label{sec:llm_ga}

A natural follow-up to Phase~C is to flip the LLM's role: instead of
evaluating the rule-based GA's winner, use the LLM as the GA's
\emph{evaluator}, and ask whether it selects different boards.

\paragraph{Setup.} 2-player only. Population 8, 5 generations,
elitism 2 → 32 candidate evaluations (\texttt{pop +
(generations-1)×(pop-elitism)}). Each candidate is scored against 5
seeded all-LLM games using the same combined objective as the
rule-based GA. The 66-dim design space, the \texttt{StandardPlayerSettings},
the dice/deck seeding, the score formula, and the \texttt{evaluate()}
function are all identical to the rule-based pipeline; only the
players are swapped.

\paragraph{Wall time.} 4h 58min end-to-end on a single 11~GB GPU.

\paragraph{Convergence and winner.}
Figure~\ref{fig:llm_ga_convergence} shows the per-evaluation score and
the best-so-far envelope. The GA found its winner at evaluation
index 15 (gen~2) with composite score $0.465$, down 18\% from
generation~0's best ($0.565$). Elitism carried the winner unchanged
through gen~3; gen~4 regressed slightly ($0.513$), consistent with
n\_seeds=5 noise on top of a real optimum.

\begin{figure}[h!]
\centering
\includegraphics[width=0.99\linewidth]{llm/fig_llm_ga_convergence.png}
\caption{\textbf{Task 2 LLM-driven GA convergence.} 32 candidate
evaluations, pop=8 × generations=5 × elitism=2, n\_seeds=5 per
candidate. The dashed line marks the rule-based GA-2p winner's
composite score for orientation; the LLM-driven GA finds a board
with lower composite under its (much smaller) game budget but the
two scores are not directly comparable; see caveats below.}
\label{fig:llm_ga_convergence}
\end{figure}

Figure~\ref{fig:llm_ga_distribution} unpacks the same 32 evaluations
as a per-generation distribution to make both selection pressure and
n\_seeds=5 evaluation noise visible. The median drops gen 0$\to$2
(selection is real); the spread within each generation is wide
(noise from the 5-seed protocol).

\begin{figure}[h!]
\centering
\includegraphics[width=0.92\linewidth]{llm/fig_llm_ga_score_distribution.png}
\caption{\textbf{LLM-driven GA: per-generation score distributions.}
Median composite score drops from gen 0 (best 0.565) to gen 2
(winner 0.465); generation~4 regresses on top of a real optimum, a
signature of n\_seeds=5 evaluation noise. The within-generation
spread is the budget cost of using LLM-only play in the inner loop.}
\label{fig:llm_ga_distribution}
\end{figure}

\paragraph{Comparison vs the rule-based GA winner.} Both winners are
44-dim continuous vectors plus a 22-bit keep-mask, but they live in
different regions of the design space. Evaluated each by its own
evaluator:
...
The LLM-driven GA found a board that is shorter (-28\%), fairer
(-17\%), and more inter-player-cash-mobile (+24\%) than the
rule-based GA winner, when each is evaluated under its own player
class. \emph{This does not mean the LLM-driven board is universally
better}: the composite scores are computed under fundamentally
different evaluators (rule-based: 1{,}000 games × 10 strategy
matchups per candidate; LLM: 5 games per candidate). What \emph{is}
meaningful is the per-component metric direction: the LLM-driven GA
amplifies the same direction the rule-based GA chose, finding a more
extreme corner of the same design space.

\begin{figure}[h!]
\centering
\includegraphics[width=0.95\linewidth]{llm/board_llm_ga_2p.png}
\caption{\textbf{The LLM-driven GA's winning board.} Rendered using
the same canonical 11×11 layout used elsewhere in the report; cells
the GA dropped (keep-mask=0) appear as REMOVED placeholders so the
reader can see which property positions the search retired.
Compared with the rule-based GA winner, this board pushes
cost/rent multipliers further from 1.0 in several groups, removes a
larger fraction of mid-cost properties, and yields the shorter,
more cash-mobile games quantified above.}
\label{fig:board_llm_ga}
\end{figure}

\paragraph{Caveats.} (1) n\_seeds=5 per candidate is noisy by
construction; some of the score gap is search-noise advantage rather
than real preference. (2) The LLM evaluator is more aggressive than
the average rule-based strategy (buy-rate by group is 0.65--1.00 even
on the GA-winner board), so the GA is implicitly optimising against
an ``aggressive-buyer-only'' eval distribution.

\paragraph{Cross-evaluation under the rule-based pool.} To stress-test
how board-level the LLM-driven GA's win is; versus how
evaluator-specific; we re-evaluated the LLM-driven winner under the
\emph{rule-based} 30-strategy pool's standard protocol (1{,}000 games
× 10 strategy matchups, both 2p and 3p). Results:
...
\toprule
                              & n\_p & score  & fairness & rounds & xfer/r \\
\midrule
Default board (identity)      & 2    & 1.463  & 0.454    & 103.9  & 49.7 \\
Rule-based GA-2p winner       & 2    & 0.817  & 0.237    & 62.5   & 74.7 \\
\textbf{LLM-driven GA-2p winner} & 2 & \textbf{1.045} & \textbf{0.379} & \textbf{45.9} & \textbf{82.4} \\
\midrule
Default board (identity)      & 3    & 1.230  & 0.412    & 110.8  & 99.4 \\
Rule-based GA-3p winner       & 3    & 0.787  & 0.361    & 64.4   & 127.0 \\
\textbf{LLM-driven GA-2p winner} & 3 & \textbf{1.015} & \textbf{0.404} & \textbf{47.1} & \textbf{160.6} \\
\bottomrule
\end{tabular}
\caption{LLM-driven GA winner re-evaluated under the rule-based
30-strategy pool. Generalises partially: beats the default board on
both 2p and 3p, but loses to the rule-based GA's own winners.}
\label{tab:llm_ga_cross_eval}
\end{table}

Three findings stand out. First, the LLM-driven winner \emph{generalises
partially}: under the rule-based pool's eval it scores 1.045 (vs
1.463 default) at 2p and 1.015 (vs 1.230 default) at 3p. The board is
a real improvement over the default; it is not just an artefact of the
LLM evaluator. Second, the rule-based GA's own 2p winner remains
better when scored on its own protocol (0.817 vs 1.045); the
search algorithms each found optima inside their evaluator's
distribution. Third, and most diagnostic: the
LLM-driven winner's \emph{fairness} balloons under the pool's eval,
from 0.20 (LLM evaluator: two identical seats, only seat-position
bias measured) to 0.379 (pool evaluator: 30 different strategy
archetypes, real strategic asymmetry). The board has \emph{exploitable
asymmetries} that the LLM evaluator missed because it played the same
strategy from every seat. Game-length effects transfer cleanly across
evaluators (rounds 43.6 LLM vs 45.9 pool; 28\% shorter than the
rule-based winner in either case); fairness effects do not.

Practical takeaway: a single-personality evaluator can drive useful
...
\emph{both} evaluators side-by-side, so the partial-generalisation
pattern is visible at a glance: the LLM-driven winner sits between
the default board and the rule-based winner under the rule-based
pool. Figure~\ref{fig:fairness_asymmetry} isolates the fairness
diagnostic; the LLM-driven winner has by far the largest gap
between its LLM-evaluator fairness ($\approx$0.20, just seat-position
bias) and its rule-based-pool fairness ($\approx$0.379), which is
exactly the failure mode the diverse pool is designed to surface.

\begin{figure}[h!]
\centering
\includegraphics[width=0.99\linewidth]{llm/fig_cross_evaluator_gap.png}
\caption{\textbf{Cross-evaluator comparison, per-component metrics.}
Each panel: 3 boards (default, rule-based GA winner, LLM-driven
GA-2p winner) under both evaluators (LLM, blue, $n{=}20$; rule-based
30-strategy pool, grey, $n{=}1000$). Top row: mean rounds. Bottom
row: fairness. \textbf{Rounds transfer.} Both evaluators agree on
the ordering of game length: default longest, optimised boards
shorter, LLM-driven GA winner shortest. \textbf{Fairness does not.}
On the LLM evaluator, fairness measures only seat-position bias
between identical seats and shows the LLM-driven GA winner as
\emph{fairer} than the rule-based winner; on the pool evaluator,
fairness measures real strategic asymmetry and the same board scores
as the \emph{least fair}. The fairness gap is the diagnostic
finding: a single-personality evaluator systematically under-counts
asymmetries that diverse strategies surface.}
\label{fig:cross_evaluator_gap}
\end{figure}

\begin{figure}[h!]
\centering
\includegraphics[width=0.95\linewidth]{llm/fig_fairness_asymmetry.png}
\caption{\textbf{Fairness gap is the diagnostic.} A single-personality
evaluator (the LLM, two identical seats) measures only seat-position
bias; a diverse pool (30 strategy archetypes) measures real
strategic asymmetry. The LLM-driven GA winner has the LARGEST gap
between these two measurements; its low fairness under the LLM
evaluator was a false negative on board fairness, hidden by
evaluator homogeneity.}
\label{fig:fairness_asymmetry}
\end{figure}

\paragraph{Decision-log analysis on the winner.} Figure~\ref{fig:llm_slices}
already shows that the LLM's behaviour shifts when playing on a GA-
optimised board (right cluster). The same analysis on the LLM-driven
GA winner specifically: 0/74 hallucinations across the 5 winning
games, buy-rate by monopoly opportunity goes
fresh=1.00, partial\_self=1.00, opp\_dominates=0.13; the model
plays the optimal strategy archetype almost mechanically on this
particular board, which is consistent with the GA having selected
for boards where the strategically-correct move is unambiguous from
\texttt{STATE} alone.
...
or 2 that look best on the dimensions of interest, and spend human
playtest time on candidates with a clear, pre-declared reason for being
tested.

\section{Conclusion}

We built a closed-loop game-design optimiser for Monopoly, but the
main result is the design workflow rather than the board it outputs.
A diverse parametric strategy pool evaluates candidate boards and
improves a combined fairness/length/draw/interactivity score by
32\%--46\% ($n{=}1000$) on both 2-player and 3-player versions,
beating random search by 16--17\% at equal evaluation budget. A
second agent class (Qwen2.5-1.5B with deterministic per-field echo
validation) provides cross-class corroboration: 80 all-LLM games on
the rule-based GA winners reproduce the same rounds-down,
transfer-up direction in both 2p and 3p, with 0/2{,}288
hallucinations once the validator is fixed. Flipping the LLM into
the evaluator role and re-running the GA produces a board that
generalises partially under the rule-based pool; better than
default in both player counts, worse than the rule-based GA's own
winner; and exposes a fairness gap (0.20 LLM eval $\to$ 0.379
pool eval) that diagnoses the limits of single-personality
evaluation. Together these results argue for a particular workflow
shape: \emph{multiple agent classes with diverse strategy
distributions, agreement-as-evidence, disagreement-as-diagnostic, and
human validation reserved for the cases where automated signals
diverge or the score is on the boundary}. The protocol around the
agent (validator, retry policy, eval distribution) is part of the
probe; it has its own failure modes and must be reported alongside
the agent itself. The framework, and its extracted MAS principles
(robust aggregation, evaluator diversity, behavioural-interactivity
metrics, variance-reduction discipline), generalises to environment
design problems where (i) the environment can be parameterised, (ii)
a diverse agent population can be assembled, and (iii) evaluation is
cheap relative to the search budget. Human validation remains the
final test of whether these directional signals transfer to real
play.

\paragraph{Reproducibility.} All experiments live under
\texttt{monopoly/logs/optimizer/} (rule-based) and
\texttt{monopoly/logs/llm\_eval/}, \texttt{monopoly/logs/optimizer\_llm/}
(LLM) as JSONL run logs and JSON summaries. Every figure and table
number can be regenerated by re-running
\texttt{scripts/optimize\_board.py},
\texttt{scripts/optimize\_board\_llm.py},
\texttt{scripts/eval\_llm\_on\_boards.py},
\texttt{scripts/cross\_eval.py},
\texttt{scripts/strategy\_heatmap.py},
\texttt{scripts/analyze\_llm\_decisions.py},
\texttt{scripts/plot\_llm\_results.py}, and
\texttt{scripts/report\_runs.py}
...
The problem we set out to solve is: \emph{find game designs that
are fair, decisive, and interactive, given that we cannot ask
thousands of humans to playtest the design space}. Each piece of
the framing was a deliberate choice, and the choice itself, not the
artefact, is the answer to this criterion.

\paragraph{Inverse framing of the standard agent-environment loop.}
Reinforcement learning fixes the environment and varies the agent.
We did the opposite: fix the agent population, vary the environment.
This makes the design space the optimisation target and treats the
agent population as a measurement instrument. The decision is
load-bearing because it forces us to ask ``what would generalise
across many strategies?'' rather than ``what does my one agent
prefer?''.

\paragraph{Multi-dimensional objective.} Design quality is
irreducibly multi-dimensional (a fair game can still be too long;
a short game can still feel one-sided; an interactive game can still
draw). We named four design qualities up front, gave each a numeric
operationalisation and a target value, and exposed every weight as
a CLI flag. Collapsing this to a single ``goodness'' score hides
which knob the optimiser is really pulling.

\paragraph{Population-as-evaluator.} A single trained agent
conflates ``the design is good'' with ``the agent likes the
design''. A diverse strategy pool factors those apart: an
improvement that survives across 30 strategies is more likely to be
about the design than the agent. Adding a second agent class
(LLM) tests whether the rule-based pool's signal generalises to a
fundamentally different reasoning style.

\paragraph{Directional signals, not human substitutes.} We did
not claim the agents predict human play. We claimed that
\emph{when the design changes, the direction the agents' metrics
move on is informative}, and that disagreement between agent
classes is itself a useful signal. This changes the standard the
work needs to meet: not ``does this match humans?'' but ``does
this give a designer something to act on?''.

\paragraph{Numerics-first commitment.} Every variable, range,
target, and budget was committed to in writing before optimisation
ran. The point is falsifiability: a description that cannot be
computed cannot be wrong, and a workflow nobody can re-derive is
not a contribution. Implementation details supporting the framing
are in Section~\ref{sec:problem}.

\subsection*{Criterion 2 -- ``How thoughtfully did the students evaluate
their idea or technical implementation?''}

The evaluation was designed to answer ``how would I know if this is
wrong?'' before any number was reported. Each move below corresponds
to a specific failure mode the evaluation is trying to rule out.

\paragraph{Anchor against a noise floor and a head-to-head
baseline.} Improvement claims are only meaningful relative to a
concrete reference. We measured the unmodified default board first
(noise floor) and then ran two search algorithms (random vs GA) at
matched evaluation budget (head-to-head), so any claim of
``optimisation worked'' has to beat both.

\paragraph{Single-objective ablations to prove non-redundancy.}
A multi-dimensional objective is only meaningful if each component
is doing real work. We re-ran the optimisation with one objective
weight at a time set to 1.0 and the rest to 0.0; if the resulting
designs collapsed onto the combined-objective optimum, we would
have evidence that some terms are decorative. They didn't, which
shows the trade-off surface is real.

\paragraph{Cross-class validation.} The rule-based pool can hide
its own quirks behind the pretence of being a generalisable
evaluator. A second agent class (Qwen2.5-1.5B with deterministic
echo validation) was added specifically as a sanity check
\emph{not subject to those quirks}. Cross-class agreement on
direction is more compelling than either class on its own; cross-
class disagreement (the fairness gap) is itself a finding.

\paragraph{Re-evaluate optima under each other's evaluators.} The
LLM-driven GA winner under the rule-based pool, and vice-versa,
exposes the kind of evaluator-distribution mismatch that would
otherwise look like a free lunch. The cross-evaluator gap on
fairness was the diagnostic finding the workflow surfaced.

\paragraph{Expose protocol failure modes rather than suppress
them.} The validator-and-retry stack around the LLM caused
retry-induced fabrication when the validator was wrong. We
documented this as a finding rather than tuning the validator until
it disappeared. A protocol whose failure modes are reported
alongside its results is auditable; one whose are not is a
benchmark trick.

\paragraph{Reproducibility as a methodological commitment.} Every
number in the paper can be regenerated from a seed and a JSONL
line; the seed schedule is recorded in each run's
\texttt{meta.json}. This is not a courtesy; it is what makes a
methodology paper testable by someone who did not write it. Full
artefact and table inventory is in Section~\ref{sec:eval}.
