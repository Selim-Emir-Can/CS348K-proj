...
\begin{abstract}
Game designers increasingly need to evaluate more candidate rule sets
than human playtesting can cover. We present a workflow for using
imperfect agents as \emph{design probes}: parameterise the game,
evaluate candidates against a diverse agent population, compare
directional predictions across agent classes, and reserve scarce
human playtests for the designs where the automated signals are
strongest or most conflicted. Monopoly is our reproducible case
study. A genetic algorithm searches a 66-dim board design space
(per-property cost/rent multipliers + a keep-mask that structurally
shortens the board) against a composite of fairness, game length,
draw rate, and inter-agent money transfer; single-objective
ablations and a $2p\!\leftrightarrow\!3p$ cross-evaluation at
$n{=}1000$ yield $\sim$40\% score reductions over the default board.
We then run two cross-class validations of the workflow. \emph{Phase
A} on a 16-cell mini board with three pre-declared knobs has
rule-based agents and a Qwen2.5-1.5B LLM agreeing on direction for
$8/12$ metric comparisons and cross-confirming a counter-intuitive
finding (doubling salary \emph{increases} draw rate). \emph{Phase
C}, completed, scales the LLM cross-check to the full 40-cell
board: 80 games of all-LLM play (2{,}288 LLM decisions, 0
hallucinations under deterministic per-field echo validation)
reproduce the rule-based GA's directional signal in both 2p and 3p,
and a mirror-image LLM-driven GA at 32 evaluations finds a board
that beats the default but loses to the rule-based winner under the
diverse-strategy pool — with a fairness gap (0.20 LLM eval $\to$
0.379 pool eval) that diagnoses a single-personality evaluator's
blind spot. The novel claim is not that the resulting Monopoly board
is universally better, nor that agents are accurate human
substitutes; rather, that automated agent feedback is
decision-useful when treated as a directional, comparative signal,
and that the validator-and-retry stack around the agent is part of
the probe and shapes its failure modes.
Code:
...
long but uneven history. Search-based game design and procedural-content
generation show that parameters can be optimised against computational
objectives; automated playtesting shows that agents can expose bugs,
dominant strategies, and balance failures; multi-agent RL often fixes
the environment and evolves increasingly strong agents. Our setting sits
between these lines of work. The \emph{environment} is the search
variable, while the \emph{agent population} is fixed, diverse, and
intentionally imperfect. The novelty is the interpretation layer: we do
not ask whether any one agent is ``the human''. We ask whether multiple
agent classes agree on the direction of a design effect, and whether
their disagreements identify the playtest conditions a designer should
spend human time on. This framing gives the designer actionable leverage
(environment parameters are cheap to change; retraining agents for every
candidate design is not) while retaining the honesty of a population-level
evaluation (the optimum must generalise across strategies, not just
overfit to one reference agent). The same framing applies to matchmaker
design, auction-parameter tuning, platform-rules optimisation, and any
other multi-agent system whose designer can instrument the environment
independently of the agents that populate it.
...
predictive of human play) is operationalised by a three-phase
validation plan with pre-declared methodology, statistics, and
falsification criteria. Each phase generates a different kind of
evidence about \emph{when} agent feedback is decision-useful and when
it is not. As of this writing, Phase A is complete and reported in
Section~\ref{sec:phase_a_results}; Phases B and C remain future work
with a concrete protocol committed below.
